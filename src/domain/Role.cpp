...
Role &Role::operator=(const Role &player) {
	if (this == &player) {
		return *this;
	}
...
void Role::setExp(int e) {
	...
	int newLevel = 1;
	for (int i = 0; i < 29; i++) {
		if (exp >= levelExp_Max[i]) {
			newLevel = i + 2;
		}
	}

	if (newLevel > level) {
		for (int currentLevel = level; currentLevel < newLevel; currentLevel++) {
			const int gainIndex = currentLevel - 1;
			setHealth_max(getHealth_max() + levelExp_Max[gainIndex]);
			setMagic_max(getMagic_max() + levelExp_Max[gainIndex]);
			setAttack(getAttack() + levelExp_Max[gainIndex] / 2);
			setDefend(getDefend() + levelExp_Max[gainIndex] / 4);
		}
		setLevel(newLevel);
	}
}
...
int Role::getLevelExp_max(int whichLevel) const {
...
const Bag &Role::getBag() const {
	return bag;
}

Skill Role::getSkill() const {
	return skill;
}
...
void Role::setSkill(const Skill& newSkill) {
	for(int i = 1;i <= 4;i++)
		...
		if (choice == 1) {
			...
			while (true) {
				...
				if (choiceGoodsId == 22 || choiceGoodsId == 23) {
					for (int i = choiceGoodsNum;i > 0;i--)
						setMagic(goods[choiceGoodsId].getAddMP() + getMagic());
