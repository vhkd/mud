...
public:
	Skill();
	~Skill() {};
	int getLevel(int whatSkill) const;		//判断技能等级（是否为零尚未习得）对应伤害
	bool addLevel(int whatSkill);	    //增加技能等级，每次一级
	void setLevel(int whatSkill,int l);		//设置技能等级

	int getLevel_Max(int whatSkill) const;	//得到技能的最高等级
	string getName(int whatSkill) const;		//得到技能名字
	string getDisc(int whatSkill) const;		//得到技能描述
	int getType(int whatSkill) const;			//得到技能类型
	int getNeedMP(int whatSkill) const;		//得到技能耗蓝
	double getAddAttack(int whatSkill) const;	//得到技能的攻击力加强
	int getRecoverHp(int whatSkill) const;	//得到技能恢复的生命值
	int getDebuff(int whatSkill) const;
